 ...
 * @license GPL 2.0
 */
define( 'SITEORIGIN_THEME_VERSION', '1.9.19' );
...
if ( ! function_exists( 'siteorigin_unwind_setup' ) ) {
	...
	function siteorigin_unwind_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters( 'siteorigin_unwind_custom_background_args', array(
...
function siteorigin_unwind_scripts() {
	...
	if (
		(
			is_home() &&
			siteorigin_setting( 'blog_featured_slider' ) &&
			siteorigin_unwind_has_featured_posts()
		) ||
		(
			is_single() &&
	...
	// Theme JavaScript.
	wp_enqueue_script(
		'siteorigin-unwind-script',
		get_template_directory_uri() . '/js/unwind' . SITEORIGIN_THEME_JS_PREFIX . '.js',
		array( 'jquery' ),
		SITEORIGIN_THEME_VERSION,
		true
	);
	...
	// Skip link focus fix.
	wp_enqueue_script(
		'siteorigin-unwind-skip-link-focus-fix',
