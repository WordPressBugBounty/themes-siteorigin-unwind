...
class SiteOrigin_Settings_Control_Widget extends WP_Customize_Control {
	...
	public function render_content() {
		...
		if ( ! class_exists( $this->widget_args['class'] ) ) {
			...
			_e( 'This field requires the Widgets Bundle plugin.', 'siteorigin-unwind' );
			echo ' ';
			printf( __( '<a href="%s">Install</a> the Widgets Bundle now.', 'siteorigin-unwind' ), 'https://wordpress.org/plugins/so-widgets-bundle/' );
			?></div>
			...
			$form = '<p><a href="" class="button-secondary so-widget-close">' . __( 'Close', 'siteorigin-unwind' ) . '</a></p>' . ob_get_clean();
			...
			$form .= '<p><a href="" class="button-secondary so-widget-close">' . __( 'Close', 'siteorigin-unwind' ) . '</a></p>';
			...
			<div class="so-settings-widget-form">
				...
				<a href="#" class="button-primary so-edit-widget"><?php esc_html_e( 'Edit Widget', 'siteorigin-unwind' ); ?></a>
			</div>
			<?php
		}
	}
